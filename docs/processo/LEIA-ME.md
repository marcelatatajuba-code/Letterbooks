...
| **Capacidade de detecção** | defeito existe desde que foi escrito; o que muda é quando alguém consegue vê-lo. **30 dos 103** defeitos da última fase foram injetados antes dela e ficaram latentes de 1h20 a 25h. |
...
| defeitos registrados | 10 | 13 | 84 |
| **achados pela usuária** | **3 de 10** | **4 de 13** | **4 de 103** |
...
**A coluna de achados pela usuária deixou de ser zero, e o motivo mudou no
...
replace view` não muda a lista de colunas de uma view que já existe, e a `feed`
começa com `l.*`, então ela muda sozinha a cada coluna nova em `leituras`.
**D73** é o motivo de D72 ter chegado até ela: **as seis provas em Postgres
começavam todas de um banco vazio.** Elas provavam que o esquema NASCE certo.
Nenhuma provava que ele ATUALIZA — e atualizar é a única coisa que acontece num
banco de verdade, que tem dados e tem a forma de alguma entrega antiga.

O padrão que liga os quatro: **a verificação media o que era fácil de montar,
não o que a pessoa faz.** Banco vazio é fácil; banco de junho com dados dentro
dá trabalho. O `provar-v5.sql` fecha esse buraco — ele fabrica a forma antiga
desfazendo o que veio depois, para que a forma antiga não vire um arquivo
parado envelhecendo ao lado do que deveria vigiar.
...
**2. "Defeitos por fase" mede DETECÇÃO, não injeção.** **30 dos 103**
...
31/08 06:__  448   V10: as 12 portas do navegador viram folhas do app
31/08 08:__  464   V11: a estante no perfil alheio, e o cartão que carimbava alheio
```

Hoje: **547 asserções** em quatro suítes, mais as checagens dos casos de
regressão, o rastreador (que não assere: mede e relata) e nove provas em
Postgres. Razão de verificação por app: razão de 0,64.

**24 de 126 defeitos** tem caso de regressão que os prende. Os outros 102 estão
...
| entregas com a squad acionada | 11 de 11 | todas |
...
| arquivo | o que é |
|---|---|
| `defeitos.csv` | os 126 defeitos, com fase, gravidade, **quem detectou** e latência |
